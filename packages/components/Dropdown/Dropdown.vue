...
const tooltipProps = computed(() => {
    return omit(props, ['items', 'hide', 'type', 'size', 'splitButton', 'hideAfterClick'])
})
...
<template>
    <div class="er-dropdown" :class="{ 'is-disabled': props.disabled }">
        <my-tooltip ref="tooltipRef" v-bind="tooltipProps" :virtual-triggering="splitButton"
            :virtual-ref="triggerRef?.ref?.value ?? null" @visible-change="$emit('visible-change', $event)">
            <my-button-group v-if="splitButton" :type="type" :size="size" :disabled="disabled">
                <my-button @click="$emit('click', $event)">
                    <slot name="default"></slot>
                </my-button>
                <my-button ref="triggerRef" icon="angle-down"></my-button>
            </my-button-group>
