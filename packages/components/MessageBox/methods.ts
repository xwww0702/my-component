...
import { createVNode, isVNode, ref, render, type Ref } from "vue";
import type { ComponentPublicInstance, VNode, VNodeProps } from "vue";

import MessageBoxConstructor from "./Message.vue";
import {
  assign,
  each,
  isFunction,
...
function initInstance(props: MessageBoxProps, container: HTMLElement) {
  ...
  const isVnodeMessage = isFunction(props?.message) || isVNode(props?.message);
  const genDeafultSlot = (msg: VNode | (() => VNode)) =>
  ...
  const vnode = createVNode(
    ...
    isVnodeMessage
      ? { default: genDeafultSlot(props.message as VNode) }
      : void 0
  );
